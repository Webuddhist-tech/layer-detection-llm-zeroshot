...
The prompt used for tsawa with both Gemini 3.1 Flash Lite and Claude Sonnet 5.

Every statistic in it was measured on the train and validation books of the tsawa dataset
(100 books, 14,829 merged spans). The test split was never looked at, and the worked examples
are copied from train and validation books.

Windows, anchors and the locator are the quotation benchmark's setup unchanged: 16,000
characters overlapping by 2,000, cut at a shad, `{spans:[{label, frame, head, tail}]}` anchors
mapped back to offsets by the locator.
