...
The prompt used for chapter (chapter and section headings) with Gemini 3.1 Flash Lite. It is the
prompt Claude used plus one paragraph, the density instruction under "How often the answer is
empty". That paragraph was added after reading Claude's test errors, so this prompt is not
