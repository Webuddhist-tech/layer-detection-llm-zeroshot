#!/usr/bin/env python3
"""
zeroshot_run.py - zero-shot span extraction for one layer (tsawa or sabche) with an LLM.

Pipeline: 16,000-character windows overlapping by 2,000 and cut at a shad, one LLM call per
window, the model returns anchors (label, head, tail, frame) and not offsets, the locator maps
the anchors back to character offsets, spans from overlapping windows are de-duplicated, and
each book is scored (IoU 0.5, greedy one-to-one, inclusive character offsets).
    ...
    python src/zeroshot_run.py --layer sabche --texts-dir data/raw_opf --books P000067 \
        --provider anthropic --model claude-sonnet-5 --out runs/sabche/claude

Raw replies are cached one file per window, so a re-run costs nothing for windows that already
have one. --stop-after N pauses after N books that needed API calls. --locate-only re-scores
the cache without any API call. API keys are read from the environment and never written
to disk.
"""
...
LAYERS = {
    "tsawa": {"label": "TSAWA", "gold": "data/tsawa_gold.csv",
...
def set_layer(name: str) -> None:
    global GOLD, MANIFEST, PROMPT, LABEL
    cfg = LAYERS[name]
    GOLD, MANIFEST, PROMPT = ROOT / cfg["gold"], ROOT / cfg["manifest"], ROOT / cfg["prompt"]
    LABEL = cfg["label"]
...
def main():
    ...
    ap.add_argument("--texts-dir", default="data/raw_opf",
                    help="directory of OpenPecha .opf checkouts (src/fetch_texts.py)")
    ap.add_argument("--out", default="runs/zeroshot")
    ap.add_argument("--prompt", default=None, help="default: <layer>/prompt.md")
    ...
    prompt = load_prompt(Path(args.prompt or PROMPT))
    ...
    for book in args.books:
        ...
        for i, c in enumerate(chunks):
            ...
            local = locate_window(c["text"], rec.get("raw", ""), LABEL)
