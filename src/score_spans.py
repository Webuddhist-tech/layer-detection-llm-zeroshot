#!/usr/bin/env python3
"""
score_spans.py - score predicted character spans against the gold spans of one layer.

Needs only offsets (no book text): data/<layer>_gold.csv, data/<layer>_books_manifest.csv
and one span directory per model, each holding <book>.json = {"spans": [[s, e]]} with
INCLUSIVE character offsets.

Per book, a prediction is correct when its character IoU with a gold span is at least 0.5,
matched greedily one to one. Totals are micro-averaged over books.

    python src/score_spans.py --layer tsawa --split test \\
        --model gemini=results/tsawa/gemini-3.1-flash-lite/test/spans \\
        --model claude=results/tsawa/claude-sonnet-5/test/spans

Rows: all books, old batch, new batch, median book. For tsawa there is also a row without
IF3ACC3E1, an interlinear book with 747 tiny gold spans that no model scores on.
--per-book adds a table with each model's F1 and false-positive count per book.
"""
...
def score(gold, pred, thr: float = 0.5) -> dict:
    ...
    for _v, gi, pi in cands:
        ...
        used_g.add(gi)
        used_p.add(pi)
    return {"tp": len(used_g), "n_pred": len(pred), "n_gold": len(gold)}
...
def main():
    ...
    manifest = {r["pecha_id"]: r for r in csv.DictReader(
        open(ROOT / f"data/{args.layer}_books_manifest.csv", encoding="utf-8"))}
    books = sorted(b for b, r in manifest.items() if r["split"] == args.split)
    gold = {b: [] for b in books}
    for r in csv.DictReader(open(ROOT / f"data/{args.layer}_gold.csv", encoding="utf-8")):
    ...
    groups = [(f"all {len(books)} books", books)]
    outlier = OUTLIER.get(args.layer)
    if outlier in books:
        groups.append((f"without {outlier}", [b for b in books if b != outlier]))
    groups.append(("old batch", [b for b in books if manifest[b]["source_batch"] == "old"]))
    groups.append(("new batch", [b for b in books if manifest[b]["source_batch"] == "new"]))

    names = list(per)
    width = 26
    print(f"layer={args.layer} split={args.split}")
    ...
    for n in names:
        m = statistics.median(
            prf(per[n][b]["tp"], per[n][b]["n_pred"], per[n][b]["n_gold"])[0] for b in books)
        out[n]["median_book_f1"] = m
        med.append(f"F1 {m:.3f}")
    print(f"{'median book':20}" + "".join(f"{c:<{width}}" for c in med))

    if args.per_book:
        print(f"\n{'book':11}{'batch':6}{'gold':>6}  "
              + "  ".join(f"{n[:18] + ' F1 / FP':>26}" for n in names))
        first = names[0]
        order = sorted(books, key=lambda b: -prf(*[per[first][b][k] for k in ("tp", "n_pred", "n_gold")])[0])
        for b in order:
            cells = []
            for n in names:
                s = per[n][b]
