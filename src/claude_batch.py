#!/usr/bin/env python3
"""
claude_batch.py - run Claude Sonnet 5 on the test windows of one layer through the Message
Batches API (half price, asynchronous). It uses the same windows, prompt and per-window cache
as src/zeroshot_run.py, so that script can score the replies. Needs the book texts
(src/fetch_texts.py) and ANTHROPIC_API_KEY.

Nothing is sent unless `create` is run with --yes.

    # 1. offline: how many windows, split into groups, estimated cost
    python src/claude_batch.py --layer sabche plan --groups 3

    # 2. dry run of one group, then submit it
    python src/claude_batch.py --layer sabche create --group g1 --books P000083 ...
    python src/claude_batch.py --layer sabche create --group g1 --books P000083 ... --yes

    # 3. wait, then download one file per window
    python src/claude_batch.py --layer sabche poll --group g1
    python src/claude_batch.py --layer sabche fetch --group g1

    # 4. score the saved replies (no API key needed)
    python src/zeroshot_run.py --layer sabche --books ... --out runs/claude/sabche/test --locate-only

The batch id is saved as soon as the batch is created, a group cannot be submitted twice, and
windows that already have a saved reply are never sent again.
"""
...
RUNNER = ROOT / "src/zeroshot_run.py"
OUT = STATE = None      # set by configure(), they depend on the layer
LAYER = None
TEXTS_DIR = ROOT / "data/raw_opf"
MODEL = "claude-sonnet-5"

# Estimated cost per window on Batches (central and bad case), from a trial batch: about
# 19,900 uncached input tokens per full window, a 3,442-token cached prompt, ~4,300 output tokens.
EST_CENTRAL, EST_HIGH = 0.042, 0.053
CUSTOM_ID = re.compile(r"^[A-Za-z0-9_-]{1,64}$")


def configure(layer: str) -> None:
    global OUT, STATE, LAYER
    LAYER = layer
    OUT = ROOT / "runs" / "claude" / layer / "test"
    STATE = ROOT / "runs" / "claude" / layer / "batches.json"
